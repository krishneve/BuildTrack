...
const NAV_ITEMS = [
  ...
  { path: '/manager/workers',     label: 'Workers',      icon: '👷' },
  { path: '/manager/attendance',  label: 'Attendance',   icon: '✓' },
  { path: '/manager/materials',   label: 'Materials',    icon: '⊞' },
  { path: '/manager/payments',    label: 'Payments',     icon: '₹' },
  { path: '/manager/invoices',    label: 'Invoices',     icon: '◻' },
          ...
          <div className="flex items-center gap-3">
            ...
            {(sidebarOpen || window.innerWidth < 1024) && (
              <div>
                ...
                <div className="text-[9px] text-amber-500 mt-0.5 uppercase tracking-widest font-black">Project Manager</div>
              </div>
            )}
          </div>
          ...
          {(sidebarOpen || window.innerWidth < 1024) ? (
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Manager</div>
              </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
        <header className="h-14 bg-[var(--bg-secondary)] border-b border-[var(--border-color)] flex items-center justify-between px-4 sm:px-6 flex-shrink-0 safe-top">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
             ...
             <div className="relative">
              <Link to="/manager/notifications" className="w-10 h-10 flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-[var(--bg-tertiary)]/50 rounded-xl transition-all">🔔</Link>
              {unread > 0 && (
            ...
            <div className="lg:flex flex-col items-end hidden">
                ...
                <span className="text-[9px] text-slate-500 uppercase font-bold">Assigned Sector</span>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto bg-[var(--bg-primary)] p-4 sm:p-8 custom-scrollbar relative">
           ...
           <div className="lg:hidden mb-6 p-4 bg-amber-500 rounded-2xl shadow-lg shadow-amber-500/10 flex items-center justify-between">
              <div>
                <div className="text-[10px] text-slate-900 font-black uppercase tracking-widest mb-0.5 opacity-60">Site Status</div>
