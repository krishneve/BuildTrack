...
const NAV_ITEMS = [
  ...
  { path: '/admin/sites',       label: 'Sites',       icon: '⬡' },
  { path: '/admin/analytics',   label: 'Analytics',   icon: '◈' },
  { path: '/admin/budget',      label: 'Budget',      icon: '◎' },
  { path: '/admin/inventory',   label: 'Inventory',   icon: '⊞' },
  { path: '/admin/invoices',    label: 'Invoices',    icon: '◻' },
  { path: '/admin/payments',    label: 'Payments',    icon: '₹' },
  { path: '/admin/ai',          label: 'AI Insights', icon: '🧠' },
          ...
          <div className="flex items-center gap-3">
            ...
            {(sidebarOpen || window.innerWidth < 1024) && (
              <div>
                ...
                <div className="text-[9px] text-amber-500 mt-0.5 uppercase tracking-widest font-black">Super Admin</div>
              </div>
            )}
          </div>
          ...
          {(sidebarOpen || window.innerWidth < 1024) ? (
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[10px] text-slate-500 uppercase font-black tracking-widest">Admin</div>
              </div>
        ...
        <header className="h-14 bg-[var(--bg-secondary)] border-b border-[var(--border-color)] flex items-center justify-between px-4 sm:px-6 flex-shrink-0 safe-top">
          <div className="flex items-center gap-4">
             ...
             <div className="hidden sm:block">
                ...
                <div className="text-xs font-bold text-[var(--text-primary)]">Admin Hub</div>
             </div>
          </div>
          ...
          <div className="flex items-center gap-3 sm:gap-4">
             ...
             <div className="relative">
              <Link to="/admin/notifications" className="w-10 h-10 flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-[var(--bg-tertiary)]/50 rounded-xl transition-all">🔔</Link>
              {unread > 0 && (
